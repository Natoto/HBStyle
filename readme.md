# soda-ios-codesnippets 
*soda-ios-codesnippets* 是一个 soda 团队日常工作中整理出来的用于 Xcode 的 iOS 通用代码片段集，其中也包含若干专用于 soda for iOS 框架的代码片段。
...
为了方便更新，建议直接将 [soda-ios-codesnippets](https://github.com/Natoto/HBStyle/tree/master/CodeSnippets) `clone` 到这个目录内。其中以 `QM_` 前缀开头的文件是通用的 Code Snippets，以 `soda_` 前缀开头的文件是专用于 `soda for iOS` 框架的代码片段。在下方的快捷键汇总里，soda 的代码片段将会以soda的形式标记出来。
...
- `replaceMethod_soda` - soda 重写当前类的 `load` 方法并用 `ReplaceMethod()` 函数替换方法的实现
...
- `setFrame_soda` - soda 使用像素对齐的 `CGRectFlatMake()` 为 `UIView` 设置 `frame`
- `setFrameX` - soda 使用 `CGRectSetX()` 修改 `UIView` 的 `frame.origin.x`
- `setFrameY` - soda 使用 `CGRectSetY()` 修改 `UIView` 的 `frame.origin.y`
- `setFrameXY` - soda 使用 `CGRectSetXY()` 修改 `UIView` 的 `frame.origin`
...
- `initSubviews` - soda 展开 `initSubviews` 方法
- `setNavigationItems` - soda 重写 `sodaCommonViewController` 里的 `setNavigationItemsIsInEditMode:animated:` 方法
- `setToolbarItems` - soda 重写 `sodaCommonViewController` 里的 `setToolbarItemsIsInEditMode:animated:` 方法
- `leftBarButtonItemImage` - soda 用 `sodaNavigationButton` 的方法创建一个用于 `navigationItem.leftBarButtonItem` 的 `UIBarButtonItem`
- `rightBarButtonItemImage` - soda 用 `sodaNavigationButton` 的方法创建一个用于 `navigationItem.rightBarButtonItem` 的 `UIBarButtonItem`
